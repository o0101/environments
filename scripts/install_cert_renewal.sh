...
(sudo crontab -l | grep "tls_renew.sh $1") || ( sudo crontab -l; echo "0 12 * * * root /root/tls_renew.sh $1 $2"; ) | sudo crontab

# test, runs every 2 minutes haha
#(sudo crontab -l | grep "tls_renew.sh $1") || ( sudo crontab -l; echo "*/2 * * * * root /root/tls_renew.sh $1 $2"; ) | sudo crontab
